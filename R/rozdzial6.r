...
library(gamlss)
library(sfsmisc) # tabele do Latexa
library(lattice) # do wykresów 
library(cplm)
library(dglm)
options(OutDec=",")
...
#title(main="Łączna liczba szkód dla pojedynczego ryzyka w województwach")
#woj1=names(table(auto1$VOIVODESHIP))
...
summary(Y)
write.csv2(summary(Y)$coefficients,"d:/Uczelnia_Nauka/Grant NCN/_Ksiazka finisz/R/Y.summary.csv")
...
weights=CLAIM_COUNT[claims.nz], family=inverse.gaussian(log), data=auto1) 
summary(Y.d)
write.csv2(summary(Y.d)$coefficients,"d:/Uczelnia_Nauka/Grant NCN/!!!Ksiązka zmiana układu po tel/R/Y.d.summary.csv")
...
summary(Y.V)
plot(Y.V)
#image(Y.V$vcov, main = 'Variance-covariance Matrix')
...
summary(N.V)
plot(N.V)
#image(N.V$vcov, main = 'Variance-covariance Matrix')
...
write.csv2(summary(NC.V)@coefs ,"d:/Uczelnia_Nauka/Grant NCN/_Ksiazka finisz/R/NC.V.fic.csv")

############ CZYNNIK NIEOBSERWOWALNY - panel
#Histogram liczby szkód w latach
